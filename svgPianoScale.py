...
# migrated inkscape 1.0 Neon22 2020
...
class SVGPianoScale (inkex.Effect):
	...
	def __init__(self):
		...
		self.arg_parser.add_argument("--firstNote",
			  type=str, default="C1",
			  dest="firstNote")
		self.arg_parser.add_argument("--lastNote",
			  type=str, default="B2",
			  dest="lastNote")
		...
		self.arg_parser.add_argument("--scale",
			  type=int,
			  dest="scale")
		self.arg_parser.add_argument("--helpSheet",
			  type=int,
			  dest="helpSheet")
		#
	...
	def calculate_size_and_positions(self):
		...
		self.doc_width  = self.svg.unittouu(self.doc.get('width'))
		self.doc_height = self.svg.unittouu(self.doc.get('height'))
		# Size of the keys
	...
	def effect(self):
		self.doc = self.document.getroot()
		self.center = self.svg.namedview.center
		self.calculate_size_and_positions()
		#
		if str(self.options.tab) == 'scale':
			t = 'translate(%s,%s)' % (self.center[0],  self.center[1])
			group = etree.SubElement(self.doc, 'g', { 'transform':t})
			self.createPiano(group)
